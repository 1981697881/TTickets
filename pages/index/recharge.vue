<template>
	<view class="page_box user-subpage user-recharge-page">
		...
		<view class="content_box">
			...
			<radio-group @change="selMoney" class="pay-box" style="margin-top: 20rpx;">
				<label class="x-bc pay-item" v-for="(item, index) in setMeal"
					:key="index">
			...
			<view class="x-c">
				<button class="cu-btn pay-btn bg-cyan" :disabled="isSubOrder || !canPay" @tap="confirmPay">确认支付 ￥{{ checkPrice }}</button>
			</view>
		</view>
...
export default {
	...
	computed: {
		...
		canPay() {
			const amount = Number(this.checkPrice);
