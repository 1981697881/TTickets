		...
		<view class="content_box">
			...
			<view class="goods-list" v-for="g in perGoodsList" :key="g.sku_price_id">
				<view class="goods-card">
					<app-mini-card :detail="g.score_goods_sku_price" :sku="g.score_goods_sku_price.sku_price_id" :type="'sku'">
						<block slot="goodsBottom">
							<view class="goods-price x-bc">
								<view class="">
									<text v-if="orderType === 'score'">{{ g.score_goods_sku_price.score }}积分+</text>
									<text>￥{{ g.score_goods_sku_price.price }}</text>
								</view>
...
export default {
	...
	methods: {
		...
		init() {
			return Promise.all([this.getPre()]);
		},
		jump(path, parmas) {
			this.$Router.push({
				path: path,
