...
export default {
	...
	computed: {
		...
		total() {
			return this.cart.reduce((acc, cur) => acc + Number(cur.goodsCount || 0) * Number(cur.PackageAmount || 0), 0);
		},
		amount() {
			const total = Number(this.total) || 0;
			const discount = Number(this.couponPrice) || 0;
	...
	async onShow() {
		...
		try {
			const parsed = typeof query.pay === 'string' ? JSON.parse(query.pay) : query.pay;
			if (!Array.isArray(parsed) || !parsed.length) throw new Error('empty cart');
			this.cart = parsed;
		} catch (error) {
			uni.showToast({ icon: 'none', title: '商品信息异常，请重新选择' });
			setTimeout(() => {
	...
	methods: {
		...
		combuy(){
			...
			const payAmount = Number(that.amount);
			if (!Number.isFinite(payAmount) || payAmount < 0) {
				uni.showToast({ icon: 'none', title: '支付金额异常' });
				return;
			}
			
			if(that.balInfo.CustID == undefined && that.balInfo.custId == undefined){
			...
			this.$api('goods.depositMixPackage', {
				...
				if (res.flag) {
					...
					uni.showToast({
						...
						success: function() {
							setTimeout(function(){
								that.jump('/pages/order/ticket', {type:'nostart'});
							}, 2000);
						}
					});
				} else {
		...
		pay() {
			...
			try {
				new AppPay(that.payType, that.cart, 'goods.payGoodsMoney', params,4,{custId: that.balInfo.custId,
					qty: that.cart[0].goodsCount+"",
					packageId: that.cart[0].packageId+"",
					couponId: that.couponId,
					placeId: that.storeInfo.v8PlaceId,
