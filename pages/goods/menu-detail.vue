		...
		<view class="detail_box app-selector">
			...
			<view class="detail-foot_box  x-f" v-if="!showSku && !showServe && detailType !== 'score'">
				...
				<view class="detail-right">
					<view class="detail-btn-box x-ac" v-if="!goodsInfo.activity"><button class="cu-btn tool-btn pay-btn" @tap="jump('/pages/goods/menu-pay', { packageId: goodsInfo.PackageId })">立即购买</button></view>
				</view>
			</view>
			<!-- 分享组件 -->
...
import fzDetailHead from './menu-children/fz-detail-head.vue';
...
.tab-detail {
	...
	.rich-box {
		/* 小程序不支持 img 标签选择器，使用 uni-parser class */
		:deep(._img),
		:deep(._image) {
			display: block;
			max-width: 100%;
